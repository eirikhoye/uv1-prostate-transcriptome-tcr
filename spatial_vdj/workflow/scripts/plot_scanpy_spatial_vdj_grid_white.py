...
global_vmax_B = np.percentile(all_B, 99)   # robust scale
...
# --- Shared colorbars for B and TRB ---
cbar_ax1 = fig.add_axes([0.35, 0.05, 0.25, 0.015])  # [left, bottom, width, height]
sm1 = ScalarMappable(norm=Normalize(vmin=0, vmax=global_vmax_B), cmap="Blues")
fig.colorbar(sm1, cax=cbar_ax1, orientation="horizontal", label="B cell counts")

cbar_ax2 = fig.add_axes([0.70, 0.05, 0.25, 0.015])
sm2 = ScalarMappable(norm=Normalize(vmin=0, vmax=global_vmax_TRB), cmap="Oranges")
fig.colorbar(sm2, cax=cbar_ax2, orientation="horizontal", label="TRB counts")

plt.savefig("spatial_grid_clean.png", dpi=600, bbox_inches="tight")
plt.show()
